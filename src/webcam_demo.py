...
def run_face_mask_dectector(threshold: float=0.19)-> None:
    ...
    while True:
        ...
        faces = face_cascade.defaultMultiScale(gray, scaleFactor=1.1, minNeighbors=5, minSize=(60,60))
        ...
        for (x,y,w,h) in faces:
            ...
            labels = "No Mask" if is_no_mask else "Mask"
            conf = prob_no_mask if is_no_mask else (1.0 - prob_no_mask)

            color = (0, 0, 225) if is_no_mask else (0, 255, 0)

            cv2.rectangle(frame, (x, y), (x + w, y+h), color, 2)
            cv2.putText(
                frame,
                f"{labels} ({conf*100:.1f}%)",
                (x, y - 100),
...
if __name__ == "__main__":
    run_face_mask_dectector(threshold=0.19)