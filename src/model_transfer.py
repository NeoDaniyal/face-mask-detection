...
from torchvision.models import resnet18, ResNet18_Weight
...
class MaskResNet18(nn.Module):
    ...
    def __init__(self, freeze_backbone: bool =False, dropout_rate: float= 0.3)->None:
        super().__init__()
        weights = ResNet18_Weight.DEFAULT
        backbone = resnet18(weights=weights)
    #Optional: freeze initial convolutional layers for feature extraction
        if freeze_backbone:
            for param in self.backbone.parameters():
                param.requires_grad = False
        # Replace default 1000-class for fc head with custom binary classification head
